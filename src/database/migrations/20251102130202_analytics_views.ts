...
export async function up(knex: Knex): Promise<void> {
    ...
    await knex.raw(`
        ...
        SELECT 
            p.*,
            COUNT(DISTINCT a.id) as actual_total_units,
            COUNT(DISTINCT CASE WHEN a.status = 'available' THEN a.id END) as available_count,
            COUNT(DISTINCT CASE WHEN a.status = 'reserved' THEN a.id END) as reserved_count,
            COUNT(DISTINCT CASE WHEN a.status = 'sold' THEN a.id END) as sold_count,
            ROUND(
            COUNT(DISTINCT CASE WHEN a.status = 'sold' THEN a.id END) * 100.0 / 
            NULLIF(COUNT(DISTINCT a.id), 0), 
            2
            ) as sold_percentage,
            (p.total_units - COUNT(DISTINCT a.id)) as units_not_listed
        FROM projects p
        LEFT JOIN apartments a ON p.id = a.project_id AND a.deleted_at IS NULL
        WHERE p.deleted_at IS NULL
        GROUP BY p.id
    `);

    await knex.raw(`
        CREATE VIEW v_lead_sources AS
        SELECT 
            utm_source,
            utm_medium,
            utm_campaign,
            COUNT(DISTINCT fs.id) as total_submissions,
            COUNT(DISTINCT lm.id) as converted_leads,
            COUNT(DISTINCT CASE WHEN lm.sync_status = 'synced' THEN lm.id END) as synced_leads,
            ROUND(COUNT(DISTINCT lm.id) * 100.0 / COUNT(DISTINCT fs.id), 2) as conversion_rate,
            COUNT(DISTINCT fs.project_id) as projects_inquired
        FROM form_submissions fs
        LEFT JOIN lead_mirrors lm ON fs.id = lm.form_submission_id
        WHERE fs.is_spam = false
    ...
    await knex.raw(`
        ...
        SELECT 
                ...
                e.registered_count,
                e.max_capacity,
                ROUND(e.registered_count * 100.0 / NULLIF(e.max_capacity, 0), 2) as capacity_percentage,
                ...
                ROUND(
                COUNT(DISTINCT CASE WHEN er.status = 'attended' THEN er.id END) * 100.0 /
                NULLIF(COUNT(DISTINCT er.id), 0),
                2
                ) as attendance_rate,
                COUNT(DISTINCT ei.id) as influencer_count,
                SUM(ei.reach_achieved) as total_reach
        FROM events e
        LEFT JOIN event_registrations er ON e.id = er.event_id
        LEFT JOIN event_influencers ei ON e.id = ei.event_id
        WHERE e.deleted_at IS NULL
        GROUP BY e.id, e.name, e.event_type, e.start_date, e.registered_count, e.max_capacity
    `);
}
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw("DROP VIEW IF EXISTS v_project_details");
  await knex.raw("DROP VIEW IF EXISTS v_lead_sources");
  await knex.raw("DROP VIEW IF EXISTS v_event_performance");
}
