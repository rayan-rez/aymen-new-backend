 ...
 * 
 * Handles photos for multiple entity types: projects, apartments, commercial properties, blog posts, events
 * Uses polymorphic relationship pattern for flexible attachment to different models
 * 
 * @module models/photo.model
 */

import { BasePolymorphicModel, PolymorphicEntity, PolymorphicQueryOptions } from "../base/polymorphic";
 ...
 * Update photo DTO
 */
export interface UpdatePhotoDto extends Partial<Omit<CreatePhotoDto, "photoableType" | "photoableId">> {}
...
export class PhotoModel extends BasePolymorphicModel<Photo, CreatePhotoDto, UpdatePhotoDto> {
  ...
  /**
   * Before create hook - validates photo data
   */
  ...
  async setCover(
    ...
    await this.unsetOtherCovers(photo.photoableType, photo.photoableId, photoId, trx);
  ...
  async duplicatePhotos(
    ...
    return this.bulkCreate(targetType, targetId, photoData, trx);
  }
  ...
  private applyPhotoFilters(
    ...
    if (options.hasExternalUrl !== undefined) {
      if (options.hasExternalUrl) {
        query = query.whereNotNull("external_url").where("external_url", "!=", "");
      } else {
        query = query.where(function() {
  ...
  /**
   * Custom column mapping for polymorphic fields
   */
